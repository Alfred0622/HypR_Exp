...
elif (args['dataset'] in ['AISHELL1', 'tedlium2_conformer']):
    recog_set = ['dev', 'test']
...
for task in recog_set:
    ...
    for data in data_json:
        if (data['utt_id'] not in index_dict.keys()):
            assert(data['utt_id'] in index_dict.keys()), f'{len(index_dict.keys())}'
        if (args['nbest'] > len(data['hyps'])):
            topk = len(data['hyps'])
        else:
            topk = args['nbest']
        for key in data.keys():
            if (key in ['ref', 'utt_id'] or data[key] is None):
                continue
            data[key] = data[key][:topk]

        data_len += topk
        data_json[index_dict[data['utt_id']]]['rescore'] = [0.0 for _ in range(topk)]
    ...
    with torch.no_grad():
        for data in tqdm(dataloader, ncols = 100):
            ...
            for i, (name, seq_index, masked_token ,nbest_index, length) in enumerate(
                zip(
                    data['utt_id'], data['seq_index'], data['masked_token'], data['nBest_index'], data['length']
                )
            ):
